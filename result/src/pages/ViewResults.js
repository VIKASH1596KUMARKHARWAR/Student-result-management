...
import { getResultsByStudentId, addResult, deleteResult } from './api'; // Import API functions
                ...
                {results.length > 0 ? (
                  results.map((result) => (
                    <tr key={`${result.id}-${result.semester}`}>
                      ...
                      <td>{result.cgpa}</td>
                      <td>
                        <Link to={`/admin/edit-result/${id}/${result.semester}`} className="btn btn-warning">
                          Edit
                        </Link>
                        <button 
